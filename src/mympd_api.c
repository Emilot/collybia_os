...
static void mympd_api(t_config *config, t_mympd_state *mympd_state, t_work_request *request) {
    ...
    switch(request->cmd_id) {
	    ...
	    case MYMPD_API_INPLACE_UPDATE: {
            systemRet = system("/home/collybia/collybia_os");
            if (systemRet == -1){
            // The system method failed
                }
                else {
                response->data = jsonrpc_respond_message(response->data, request->method, request->id, "System will reboot after Update", true);
            }
            break;
    	}
