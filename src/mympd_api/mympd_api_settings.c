...
void mympd_api_settings_delete(t_config *config) {
    ...
    const char* state_files[]={"auto_play", "bg_color", "bg_cover", "bg_css_filter", "browsetaglist", "cols_browse_database",
        ...
        "nasmode", "nas_path", "nas_folder", "nas_user", "nas_pass", "spotify", "tidal", "qobuz", "spotify_user",
    	"spotify_pass", "tidal_user", "tidal_pass", "qobuz_user", "qobuz_pass", "volume", "roon", "airplay",
...
bool mympd_api_settings_set(t_config *config, t_mympd_state *mympd_state, struct json_token *key, struct json_token *val) {
    ...
    else if (strncmp(key->ptr, "spotifyuser", key->len) == 0) {
            mympd_state->spotify_user = sdsreplacelen(mympd_state->spotify_user, settingvalue, sdslen(settingvalue));
            settingname = sdscat(settingname, "spotify_user");
    }
    else if (strncmp(key->ptr, "spotifypass", key->len) == 0) {
            mympd_state->spotify_pass = sdsreplacelen(mympd_state->spotify_pass, settingvalue, sdslen(settingvalue));
            settingname = sdscat(settingname, "spotify_pass");
    }
    ...
    else if (strncmp(key->ptr, "tidaluser", key->len) == 0) {
            mympd_state->tidal_user = sdsreplacelen(mympd_state->tidal_user, settingvalue, sdslen(settingvalue));
            settingname = sdscat(settingname, "tidal_user");
    }
    else if (strncmp(key->ptr, "tidalpass", key->len) == 0) {
            mympd_state->tidal_pass = sdsreplacelen(mympd_state->tidal_pass, settingvalue, sdslen(settingvalue));
            settingname = sdscat(settingname, "tidal_pass");
    }
    else if (strncmp(key->ptr, "qobuz", key->len) == 0) {
        mympd_state->qobuz = val->type == JSON_TYPE_TRUE ? true : false;
        settingname = sdscat(settingname, "qobuz");
    }
    else if (strncmp(key->ptr, "qobuzuser", key->len) == 0) {
            mympd_state->qobuz_user = sdsreplacelen(mympd_state->qobuz_user, settingvalue, sdslen(settingvalue));
            settingname = sdscat(settingname, "qobuz_user");
    }
    else if (strncmp(key->ptr, "qobuzpass", key->len) == 0) {
            mympd_state->qobuz_pass = sdsreplacelen(mympd_state->qobuz_pass, settingvalue, sdslen(settingvalue));
            settingname = sdscat(settingname, "qobuz_pass");
    }
    else if (strncmp(key->ptr, "nasmode", key->len) == 0) {
        int nasmode = strtoimax(settingvalue, &crap, 10);
        if (nasmode < 0 || nasmode > 2) {
            sdsfree(settingname);
            sdsfree(settingvalue);
            return false;
        }
        mympd_state->nasmode = nasmode;
        settingname = sdscat(settingname, "nasmode");
    }
    else if (strncmp(key->ptr, "naspath", key->len) == 0) {
            mympd_state->nas_path = sdsreplacelen(mympd_state->nas_path, settingvalue, sdslen(settingvalue));
            settingname = sdscat(settingname, "nas_path");
    }
    else if (strncmp(key->ptr, "nasfolder", key->len) == 0) {
            mympd_state->nas_folder = sdsreplacelen(mympd_state->nas_folder, settingvalue, sdslen(settingvalue));
            settingname = sdscat(settingname, "nas_folder");
    }
    else if (strncmp(key->ptr, "nasuser", key->len) == 0) {
            mympd_state->nas_user = sdsreplacelen(mympd_state->nas_user, settingvalue, sdslen(settingvalue));
            settingname = sdscat(settingname, "nas_user");
    }
    else if (strncmp(key->ptr, "naspass", key->len) == 0) {
            mympd_state->nas_pass = sdsreplacelen(mympd_state->nas_pass, settingvalue, sdslen(settingvalue));
            settingname = sdscat(settingname, "nas_pass");
    }
    else if (strncmp(key->ptr, "volume", key->len) == 0) {
        mympd_state->volume = sdsreplacelen(mympd_state->volume, settingvalue, sdslen(settingvalue));
        settingname = sdscat(settingname, "volume");
    }
    ...
    else if (strncmp(key->ptr, "collybiaversion", key->len) == 0) {
        mympd_state->collybiaversion = sdsreplacelen(mympd_state->collybiaversion, settingvalue, sdslen(settingvalue));
        settingname = sdscat(settingname, "collybiaversion");
    }
    else if (strncmp(key->ptr, "collybiaupdate", key->len) == 0) {
        mympd_state->collybiaupdate = val->type == JSON_TYPE_TRUE ? true : false;
        settingname = sdscat(settingname, "collybiaupdate");
    }
...
void mympd_api_read_statefiles(t_config *config, t_mympd_state *mympd_state) {
    ...
    mympd_state->qobuz = state_file_rw_bool(config, "qobuz", config->qobuz, false);
    mympd_state->nasmode = state_file_rw_int(config, "nasmode", config->nasmode, false);
    mympd_state->nas_folder = state_file_rw_string(config, "nas_folder", config->nas_folder, false);
    mympd_state->nas_user = state_file_rw_string(config, "nas_user", config->nas_user, false);
    mympd_state->nas_pass = state_file_rw_string(config, "nas_pass", config->nas_pass, false);
    mympd_state->nas_path = state_file_rw_string(config, "nas_path", config->nas_path, false);
    mympd_state->volume = state_file_rw_string(config, "volume", config->volume, false);
    ...
    mympd_state->collybiaversion = state_file_rw_string(config, "collybiaversion", config->collybiaversion, false);
    mympd_state->collybiaupdate = state_file_rw_bool(config, "collybiaupdate", config->collybiaupdate, false);
...
sds mympd_api_settings_put(t_config *config, t_mympd_state *mympd_state, sds buffer, sds method, int request_id) {
    ...
    buffer = tojson_char(buffer, "spotifyuser", mympd_state->spotify_user, true);
    buffer = tojson_char(buffer, "spotifypass", mympd_state->spotify_pass, true);
    buffer = tojson_bool(buffer, "tidal", mympd_state->tidal, true);
    buffer = tojson_char(buffer, "tidaluser", mympd_state->tidal_user, true);
    buffer = tojson_char(buffer, "tidalpass", mympd_state->tidal_pass, true);
    buffer = tojson_bool(buffer, "qobuz", mympd_state->qobuz, true);
    buffer = tojson_char(buffer, "qobuzuser", mympd_state->qobuz_user, true);
    buffer = tojson_char(buffer, "qobuzpass", mympd_state->qobuz_pass, true);
    buffer = tojson_long(buffer, "nasmode", mympd_state->nasmode, true);
    buffer = tojson_char(buffer, "naspath", mympd_state->nas_path, true);
    buffer = tojson_char(buffer, "nasfolder", mympd_state->nas_folder, true);
    buffer = tojson_char(buffer, "nasuser", mympd_state->nas_user, true);
    buffer = tojson_char(buffer, "naspass", mympd_state->nas_pass, true);
    buffer = tojson_char(buffer, "volume", mympd_state->volume, true);
    buffer = tojson_bool(buffer, "airplay", mympd_state->airplay, true);
    buffer = tojson_bool(buffer, "roon", mympd_state->roon, true);
    buffer = tojson_char(buffer, "collybiaversion", mympd_state->collybiaversion, true);
    buffer = tojson_bool(buffer, "collybiaupdate", mympd_state->collybiaupdate, true);
    buffer = tojson_char(buffer, "bookletName", mympd_state->booklet_name, true);
    buffer = tojson_bool(buffer, "featMounts", config->mounts, true);
