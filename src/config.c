...
static int mympd_inihandler(void *user, const char *section, const char *name, const char* value) {
    ...
    else if (MATCH("collybia", "tidal")) {
        p_config->tidal = strcmp(value, "true") == 0 ? true : false;
    }
    else if (MATCH("collybia", "qobuz")) {
        p_config->qobuz = strcmp(value, "true") == 0 ? true : false;
    }
    else if (MATCH("collybia", "nasmode")) {
        p_config->nasmode = strtoimax(value, &crap, 10);
        if (p_config->nasmode < 0 || p_config->nasmode > 2) {
            LOG_WARN("Invalid nasmode %d", p_config->nasmode);
            p_config->nasmode = 0;
        }
    }
    else if (MATCH("collybia", "naspath")) {
        p_config->nas_path = sdsreplace(p_config->nas_path, value);
    }
    else if (MATCH("collybia", "nasfolder")) {
        p_config->nas_folder = sdsreplace(p_config->nas_folder, value);
    }
    else if (MATCH("collybia", "nasuser")) {
        p_config->nas_user = sdsreplace(p_config->nas_user, value);
    }
    else if (MATCH("collybia", "naspass")) {
        p_config->nas_pass = sdsreplace(p_config->nas_pass, value);
    }
    else if (MATCH("collybia", "spotifyuser")) {
        p_config->spotify_user = sdsreplace(p_config->spotify_user, value);
    }
    else if (MATCH("collybia", "spotifypass")) {
        p_config->spotify_pass = sdsreplace(p_config->spotify_pass, value);
    }
    else if (MATCH("collybia", "tidaluser")) {
        p_config->tidal_user = sdsreplace(p_config->tidal_user, value);
    }
    else if (MATCH("collybia", "tidalpass")) {
        p_config->tidal_pass = sdsreplace(p_config->tidal_pass, value);
    }
    else if (MATCH("collybia", "qobuzuser")) {
        p_config->qobuz_user= sdsreplace(p_config->qobuz_user, value);
    }
    else if (MATCH("collybia", "qobuzpass")) {
        p_config->qobuz_pass = sdsreplace(p_config->qobuz_pass, value);
    }
    else if (MATCH("collybia", "volume")) {
        p_config->volume = sdsreplace(p_config->volume, value);
    }
    ...
    else if (MATCH("collybia", "collybiaversion")) {
        p_config->collybiaversion = sdsreplace(p_config->collybiaversion, value);
    }
    else if (MATCH("collybia", "collybiaupdate")) {
        p_config->collybiaupdate = strcmp(value, "true") == 0 ? true : false;
    }
    else {
...
static void mympd_get_env(struct t_config *config) {
    const char *env_vars[]={"MPD_HOST", "MPD_PORT", "MPD_PASS", "MPD_MUSICDIRECTORY",
        ...
        "THEME_LOCALE", "COLLYBIA_NASMODE", "COLLYBIA_NASPATH", "COLLYBIA_NASUSER", "COLLYBIA_NASPASS", "COLLYBIA_SPOTIFY", "COLLYBIA_TIDAL", "COLLYBIA_QOBUZ",
	    "COLLYBIA_SPOTIFYUSER", "COLLYBIA_SPOTIFYPASS", "COLLYBIA_TIDALUSER", "COLLYBIA_TIDALPASS", "COLLYBIA_QOBUZUSER", "COLLYBIA_QOBUZPASS", "COLLYBIA_VOLUME",
	    "COLLYBIA_ROON", "COLLYBIA_AIRPLAY", "COLLYBIA_COLLYBIAVERSION", "COLLYBIA_COLLYBIAUPDATE",
...
void mympd_free_config(t_config *config) {
    ...
    sdsfree(config->nas_user);
    sdsfree(config->nas_pass);
    sdsfree(config->nas_path);
    sdsfree(config->nas_folder);
    sdsfree(config->volume);
    sdsfree(config->collybiaversion);
...
void mympd_config_defaults(t_config *config) {
    ...
    config->tidal = false;
    config->tidal_user = sdsnew("Tidal username");
    config->tidal_pass = sdsnew("Tidal password");
    config->spotify = false;
    config->spotify_user = sdsnew("Spotify username");
    config->spotify_pass = sdsnew("Spotify password");
    config->qobuz = false;
    config->qobuz_user = sdsnew("Qobuz username");
    config->qobuz_pass = sdsnew("Qobuz password");
    config->nasmode = 0;
    config->nas_user = sdsnew("NAS username");
    config->nas_pass = sdsnew("NAS password");
    config->nas_path = sdsnew("NAS ip address");
    config->nas_folder = sdsnew("NAS Music folder");
    config->volume = sdsnew("disabled");
    config->roon = false;
    config->airplay = false;
    config->collybiaversion = sdsnew(COLLYBIA_VERSION);
    config->collybiaupdate = false;
